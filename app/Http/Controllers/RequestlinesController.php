...
class RequestlinesController extends Controller
{
    ...
    public function update(Request $request,$idl)
    {
        ...
        if (auth()->user()->role == 4) {
            if ($rl->status == 1) {
                return redirect('requestheader/'.$id.'/requestlines')->with('success','Data Berhasil Di Ubah');
            }
        }

        if ($rl->status == 1) {
                return redirect('requestheader/'.$id.'/requestlines')->with('success','Requestlines Berhasil Di Analysis');
            }
        
        return redirect('requestheader/'.$id.'/requestlines')->with('success','Status Di Requestlines Berhasil Di Ubah');
    }
