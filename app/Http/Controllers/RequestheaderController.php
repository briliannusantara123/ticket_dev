...
class RequestheaderController extends Controller
{
     public function index(Request $request)
    {
        ...
        if ($st != "" && $pr != "") {
            ...
            $rh = DB::table('request_header')
                        ...
                        ->where('request_header.status',$st)
                        ->where('request_header.priority',$pr)
                        ->orderBy('request_header.id','DESC')
                        ->get();
        }
        if (auth()->user()->role == 4) {
            $rh = DB::table('request_header')
    	...
    	return view('requestheader.index',compact('rh','company','datauser','st','pr'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if (auth()->user()->role == 3) {
            ...
            $rh->email_consultant = auth()->user()->email;
        ...
        \Mail::raw($request->company.' Membuat Request Header Dengan Description '.$request->description, function($message) use($request,$rh){
            $message->to($rh->email_consultant);
    ...
    public function requestlines_post(Request $request)
    {
        ...
        if ($request->gambar == !NULL) {
            ...
            $requestlines = Requestlines::create([
            'code'             => $request->code,
           ...
           \Mail::raw('Client Membuat Masalah Baru Di Requestlines dengan description '.$request->description, function($message) use($request,$rh){
            $message->to($rh->email_consultant);
