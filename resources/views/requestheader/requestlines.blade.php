						...
						<div class="panel">
							<div class="panel-heading">
								...
								<div class="panel-body">
									...
									@if($rh->status == 6)
									<a href="/ticket/requestheader" class="btn btn-danger">Kembali</a>
									@else	
									...
									<table class="table table-hover" id="myTable">
										...
										@if($rl->count())
											...
											<tr>
												...
												<td>{{$rl->nama}}</td>
												@if($rl->ticket_paid == 1)
													<td>{{$rl->nama_projects}} <i class="fas fa-check" title="Chas Ticket"></i></td>
												@else
						                        ...
						                        @if($rl->status == 6)
						                          ...
						                          <td><a href="/ticket/requestlines/{{$rl->idl}}/mtesting"><p style="background-color: lightgreen; border-radius: 50px;text-align:center;">Testing</p></a></td>
						                        @endif
						                        @if($rl->status == 7)
												...
												<td>
													...
													@else
													<a href="/ticket/requestlines/{{$rl->idl}}/edit" class="btn btn-warning mt-3">View</a>
													@endif
														
														@if(auth()->user()->role == 2)
														@if($rl->status == 1)
														<a href="/ticket/requestlines/{{$rl->idl}}/analysis" class="btn btn-primary mt-3" onclick="return confirm('Yakin Akan Menganalysis Data Requestlines Tersebut?');">Analysis</a>
														<a href="/ticket/requestlines/{{$rl->idl}}/delete" class="btn btn-danger mt-3" onclick="return confirm('Yakin Akan Mengcancel Data Requestlines Tersebut?');">Cancel</a>
														@endif
														@endif
														@if(auth()->user()->role == 3)
														@if($rl->status == 1)
														...
														@if($rl->status == 5)
														 @if($rl->id_developer == Null)
															<a href="/ticket/requestlines/{{$rl->idl}}/done" class="btn btn-success mt-3" onclick="return confirm('Yakin Akan Mengubah Status Requestlines Tersebut Menjadi Done?');">Done Consultant</a>
														 @endif
														@endif
														@endif
														@if(auth()->user()->role == 4)
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
                <form action="/ticket/requestlines/post" method="POST" enctype="multipart/form-data">
              {{csrf_field()}}
